...
//           {/* Modal Box */}
//           <div className='bg-[#0d0d0f]/90 text-white p-6 rounded-xl shadow-xl w-[90%] max-w-sm z-10 pointer-events-auto backdrop-blur-md'>
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed md:static left-0 top-0 h-screen z-40 overflow-hidden
          bg-[#0d0d0f]/80 border-r border-gray-800
          transition-all duration-300 flex flex-col
          ${isSidebarOpen ? 'w-64' : 'w-0 md:w-20'}
          ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
        `}
              ...
              {chatHistory.length === 0 ? (
                ...
                chatHistory.map((chat) => (
                  <button
                    key={chat.id}
                    onClick={() => setActiveChat({ ...chat, messages: [] })}
                    className={`w-full flex items-center gap-3 p-3 rounded-lg text-sm transition ${
                      activeChat?.id === chat.id
                        ? 'bg-blue-600/50 text-white'
                        : 'text-gray-300 hover:bg-gray-800/50 hover:text-white'
                    }`}
      ...
      {showLogoutModal && (
        <div className='fixed inset-0 z-50 flex items-center justify-center pointer-events-auto'>
          {/* Semi-transparent background (not fully blurred) */}
          <div
            onClick={() => setShowLogoutModal(false)}
          ...
          {/* Modal content */}
          <div className='relative z-10 bg-[#0d0d0f]/90 p-6 rounded-2xl w-[90%] max-w-sm border border-gray-700 shadow-lg text-white'>
